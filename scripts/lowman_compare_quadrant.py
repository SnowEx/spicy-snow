...
os.makedirs(Path('/Users/zachkeskinen/Documents/spicy-snow/lowman').expanduser(), exist_ok = True)
...
for lon_min, lat_min in product(range(-117, -113), range(43, 46)):
    ...
    out_nc = Path(f'/Users/zachkeskinen/Documents/spicy-snow/lowman/spicy-lowman_{lon_min}-{lon_min + 1}_{lat_min}-{lat_min + 1}.nc').expanduser()
    spicy_ds = retrieve_snow_depth(area = area, dates = dates, 
                                work_dir = Path('/Users/zachkeskinen/Documents/spicy-snow/lowman/data').expanduser(), 
